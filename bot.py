...
from top_p2e import fetch_top_p2e
...
def main_menu():
    ...
    markup.row(KeyboardButton("/info"), KeyboardButton("/airdrops"))
    markup.row(KeyboardButton("/autogames"), KeyboardButton("/top"))
    return markup
...
def send_welcome(message):
    ...
    bot.send_message(
        ...
        "Выберите команду через меню ниже:",
...
def top_cmd(message):
    log.info(f"/top вызвана пользователем {message.chat.id}")
    bot.send_chat_action(message.chat.id, 'typing')
    games = fetch_top_p2e(limit=5)
    if not games:
        bot.reply_to(message, "😕 Не удалось получить список топовых проектов.")
        return

    text = "🏆 *ТОП-5 P2E игр сейчас:*\n\n"
